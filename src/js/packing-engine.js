...
window.PackingEngine = (function() {
  ...
  function calcEnumPacking(crateL, crateW, crateH, boxL, boxW, boxH, gap, allowRotate, keepUpright, timeLimit) {
    ...
    for (var ri = 0; ri < rots.length; ri++) {
      ...
      var t0 = Date.now(), tMax = zc >= 3 ? 1000 : timeLimit, stall = 0;
  ...
  // 数学等价: max a+b s.t. a*d1 + b*d2 ≤ target, a≥0, b≥0
  // 单次遍历 a 即可完备枚举所有 (a,b) 组合
  ...
  function calcPacking(crateL, crateW, crateH, boxL, boxW, boxH, gap, layerGap, allowRotate, keepUpright, options) {
    var enumTimeLimit = (options && options.enumTimeLimit) || 15000;
  ...
  // 单次混装计算（核心逻辑）
  function calcMixedPackingOnce(crateL, crateW, crateH, boxConfigs, gap, allowRotate, sortOrder, prePlaced) {
    const cL = crateL - gap * 2;
    const cW = crateW - gap * 2;
    const cH = crateH - gap * 2;
    if (cL <= 0 || cW <= 0 || cH <= 0) return null;
    ...
    // 预占空间：将已放置的主导纸箱位置从空间树中扣除
    if (prePlaced && prePlaced.length > 0) {
      for (var ppi = 0; ppi < prePlaced.length; ppi++) {
        var pp = prePlaced[ppi];
        var newSpaces = [];
        for (var spi = 0; spi < spaces.length; spi++) {
          var sp = spaces[spi];
          if (sp.x < pp.x + pp.l && sp.x + sp.l > pp.x &&
              sp.y < pp.y + pp.w && sp.y + sp.w > pp.y &&
              sp.z < pp.z + pp.h && sp.z + sp.h > pp.z) {
            var splits = splitSpaceFull(sp, pp.x, pp.y, pp.z, pp.l, pp.w, pp.h);
            for (var si = 0; si < splits.length; si++) newSpaces.push(splits[si]);
          } else {
            newSpaces.push(sp);
          }
        }
      ...
      spaces.sort(function(a, b) { return (b.l * b.w * b.h) - (a.l * a.w * a.h); });
    }
  ...
  function exploreCombinations(crateL, crateW, crateH, boxConfigs, gap, allowRotate, prePlaced) {
    prePlaced = prePlaced || [];
    const cL = crateL - gap * 2;
    const cW = crateW - gap * 2;
    const cH = crateH - gap * 2;
    ...
    function tryPlaceSequence(sequence) {
      ...
      // 预占空间
      if (prePlaced && prePlaced.length > 0) {
        for (var pi = 0; pi < prePlaced.length; pi++) {
          var ppe = prePlaced[pi];
          var nsp = [];
          for (var xi = 0; xi < sps.length; xi++) {
            var s = sps[xi];
            if (s.x < ppe.x + ppe.l && s.x + s.l > ppe.x &&
                s.y < ppe.y + ppe.w && s.y + s.w > ppe.y &&
                s.z < ppe.z + ppe.h && s.z + s.h > ppe.z) {
              var spl = splitSpaceFull(s, ppe.x, ppe.y, ppe.z, ppe.l, ppe.w, ppe.h);
              for (var qi = 0; qi < spl.length; qi++) nsp.push(spl[qi]);
            } else {
              nsp.push(s);
            }
          }
        ...
        sps.sort(function(a, b) { return (b.l * b.w * b.h) - (a.l * a.w * a.h); });
      }
  ...
  function calcMixedPacking(crateL, crateW, crateH, boxConfigs, gap, allowRotate, retryCount, strategy, options, prePlaced) {
    prePlaced = prePlaced || [];
    var enumTimeLimit = (options && options.enumTimeLimit) || 15000;
    ...
    if (boxConfigs.length === 1 && unconstrainedBoxes.length === 1) {
      ...
      const keepUpright = bc.box.keepUpright === true;
      const singleResult = calcPacking(crateL, crateW, crateH, bc.box.l, bc.box.w, bc.box.h, gap, 0, allowRotate, keepUpright, options);
      if (singleResult) {
        // 转换为混装模式的结果格式
    ...
    // 第一轮：体积降序（默认策略）
    let bestResult = calcMixedPackingOnce(crateL, crateW, crateH, boxConfigs, gap, allowRotate, null, prePlaced);
    ...
    if (isUtilStrategy) {
      ...
      if (freeBoxCount >= 2 && freeBoxCount <= 3) {
        const explorerResult = exploreCombinations(crateL, crateW, crateH, boxConfigs, gap, allowRotate, prePlaced);
    ...
    for (let r = 1; r < rounds; r++) {
      ...
      const candidate = calcMixedPackingOnce(crateL, crateW, crateH, boxConfigs, gap, allowRotate, shuffledByVol, prePlaced);
      ...
      if (remZ > 0) {
        ...
        if (dem2.length > 0) {
          ...
          for (var dk2 = 0; dk2 < dem2.length; dk2++) {
            ...
            // 尝试每种朝向，选碰撞后剩余最多的
            var bestValidCount = 0, bestValidFill = null, bestValidPositions = null;
            for (var ri3 = 0; ri3 < rots3.length; ri3++) {
              ...
              var validPos = [];
              for (var x3 = 0; x3 < fx3; x3++) {
                for (var y3 = 0; y3 < fy3; y3++) {
                  var cand = {
                    x: g2 + x3 * rl3, y: g2 + y3 * rw3, z: maxTop,
                    l: rl3, w: rw3, h: rh3
                  };
                  // 检查是否与已有纸箱重叠
                  ...
                  // 也检查与本次已放入的顶填纸箱重叠
                  if (!hasOverlap) {
                    for (var vi = 0; vi < validPos.length; vi++) {
                      if (ov3dTop(cand, validPos[vi])) { hasOverlap = true; break; }
                    }
                  }
                  if (!hasOverlap) validPos.push(cand);
                }
              }
              if (validPos.length > bestValidCount) {
                bestValidCount = validPos.length;
                bestValidFill = { rl: rl3, rw: rw3, rh: rh3 };
                bestValidPositions = validPos;
              }
            }

            if (bestValidPositions && bestValidCount > 0) {
              var actual = Math.min(bestValidCount, demItem.remaining);
              if (actual > 0) {
                var origD = [bc4.box.l, bc4.box.w, bc4.box.h];
                var isRot = !(bestValidFill.rl === origD[0] && bestValidFill.rw === origD[1] && bestValidFill.rh === origD[2]);
                for (var vi2 = 0; vi2 < actual; vi2++) {
                  var vp = bestValidPositions[vi2];
                  pl.push({
                    x: vp.x, y: vp.y, z: vp.z,
                    l: vp.l, w: vp.w, h: vp.h,
                    boxIdx: demItem.idx,
                    color: bc4.box.color,
                    name: bc4.box.name,
                    rotated: isRot,
    ...
    if (allowRotate && bestResult && bestResult.totalCount > 0) {
      ...
      if (enumResults.length > 0) {
        ...
        var targetedSort = targetedConfigs.map(function(_, i) { return i; }).sort(function(a, b) {
          var aReal = boxConfigs[a].qty !== null ? 0 : 1; // 0=真正需求, 1=枚举推导
          var bReal = boxConfigs[b].qty !== null ? 0 : 1;
          if (aReal !== bReal) return aReal - bReal;
          var volA = targetedConfigs[a].box.l * targetedConfigs[a].box.w * targetedConfigs[a].box.h;
          var volB = targetedConfigs[b].box.l * targetedConfigs[b].box.w * targetedConfigs[b].box.h;
          return volB - volA;
        });
        var ta = calcMixedPackingOnce(crateL, crateW, crateH, targetedConfigs, gap, allowRotate, targetedSort);
  ...
  // 反推对比：对多种木箱尺寸运行算法，给出推荐
  // 单纸箱 → calcPacking 优化枚举；多纸箱 → 主导纸箱 calcPacking + 次要 calcMixedPacking(prePlaced)
  // 混装失败时回退到 FFD
  function calcReverseCompare(crateList, boxConfigs, gap, allowRotate, options) {
    var useOptimized = boxConfigs.length === 1;

    // 辅助：根据 calcPacking 结果构建一个 crate 对象
    function makeCrateEntry(crateSpec, packR, box, boxCount) {
      var usedPositions = packR.positions.slice(0, boxCount);
      var boxVol = usedPositions.reduce(function(s, p) { return s + p.l * p.w * p.h; }, 0);
      var iL = crateSpec.l - gap * 2;
      var iW = crateSpec.w - gap * 2;
      var iH = crateSpec.h - gap * 2;
      var fullVol = iL * iW * iH;
      var tw = boxCount * (parseFloat(box.weight) || 0);
      return {
        crate: { l: crateSpec.l, w: crateSpec.w, h: crateSpec.h },
        boxes: usedPositions.map(function(pos) { return { boxIdx: 0, box: box, pos: pos }; }),
        totalCount: boxCount,
        utilRate: fullVol > 0 ? boxVol / fullVol : 0,
        totalWeight: tw,
        maxWeight: crateSpec.maxWeight || 0,
        weightRate: crateSpec.maxWeight > 0 ? tw / crateSpec.maxWeight : 0,
        displayCrateL: crateSpec.l,
        displayCrateW: crateSpec.w,
        displayCrateH: crateSpec.h
      };
    }

    // 辅助：合并 dominant + mix 为单个 crate
    function makeHybridCrate(crateSpec, dominantPositions, dominantBox, dominantCount, mixResult, secondaryConfigs) {
      var domBoxes = dominantPositions.slice(0, dominantCount).map(function(pos) {
        return { boxIdx: 0, box: dominantBox, pos: pos };
      });
      var mixBoxes = [];
      if (mixResult && mixResult.placed && mixResult.placed.length > 0) {
        mixBoxes = mixResult.placed.map(function(p) {
          var sc = secondaryConfigs[p.boxIdx];
          return { boxIdx: p.boxIdx, box: sc ? sc.box : (p.name ? { l: p.l, w: p.w, h: p.h, name: p.name, color: p.color } : dominantBox), pos: p };
        });
      }
      var allBoxes = domBoxes.concat(mixBoxes);
      var totalCount = allBoxes.length;
      var boxVol = allBoxes.reduce(function(s, b) { return s + b.pos.l * b.pos.w * b.pos.h; }, 0);
      var iL = crateSpec.l - gap * 2;
      var iW = crateSpec.w - gap * 2;
      var iH = crateSpec.h - gap * 2;
      var fullVol = iL * iW * iH;
      var tw = allBoxes.reduce(function(s, b) { return s + (parseFloat(b.box.weight) || 0); }, 0);
      return {
        crate: { l: crateSpec.l, w: crateSpec.w, h: crateSpec.h },
        boxes: allBoxes,
    ...
    for (var i = 0; i < crateList.length; i++) {
      ...
      var res;
      if (useOptimized) {
        // === 单纸箱类型：calcPacking（现有逻辑，不变）===
        var bc = boxConfigs[0];
        var keepUpright = !!(bc.box && bc.box.keepUpright);
        var packR = calcPacking(c.l, c.w, c.h, bc.box.l, bc.box.w, bc.box.h, gap, 0, allowRotate, keepUpright, options);
        if (!packR || packR.count === 0) {
          res = calcReverse(c.l, c.w, c.h, boxConfigs, gap, allowRotate, c.maxWeight || 0);
        } else {
          var maxPerCrate = packR.count;
          var qtyVal = bc.qty;
          var totalDemand = (qtyVal === null || qtyVal === undefined || qtyVal === '')
            ? null : Math.max(1, parseInt(qtyVal) || 1);

          if (totalDemand === null) {
            var entry = makeCrateEntry(c, packR, bc.box, maxPerCrate);
            res = {
              crates: [entry],
              totalCrates: 1,
              totalBoxCount: maxPerCrate,
              totalUtilRate: entry.utilRate,
              crateL: c.l, crateW: c.w, crateH: c.h,
              gap: gap
            };
          } else {
            var numFullCrates = Math.floor(totalDemand / maxPerCrate);
            var lastCount = totalDemand % maxPerCrate;
            var crates = [];
            for (var j = 0; j < numFullCrates; j++) {
              crates.push(makeCrateEntry(c, packR, bc.box, maxPerCrate));
            }
            if (lastCount > 0) {
              crates.push(makeCrateEntry(c, packR, bc.box, lastCount));
            }
            var tbc = crates.reduce(function(s, cr) { return s + cr.totalCount; }, 0);
            var tv = crates.reduce(function(s, cr) { return s + cr.totalCount * (bc.box.l * bc.box.w * bc.box.h); }, 0);
            var iL = c.l - gap * 2, iW = c.w - gap * 2, iH = c.h - gap * 2;
            var fullVol = iL * iW * iH;
            var tc = crates.length;
            var tur = tc > 0 && fullVol > 0 ? tv / (fullVol * tc) : 0;
            res = {
              crates: crates,
              totalCrates: tc,
              totalBoxCount: tbc,
        ...
        // === 多纸箱类型：主导 calcPacking + 次要 calcMixedPacking(prePlaced) ===
        // 1) 选择主导纸箱：需求体积(qty×volume)最大的
        var dominantIdx = 0;
        var maxDemandVol = 0;
        for (var di = 0; di < boxConfigs.length; di++) {
          var dq = boxConfigs[di].qty;
          var dd = (dq === null || dq === undefined || dq === '') ? 1e9 : Math.max(1, parseInt(dq) || 1);
          var dv = dd * boxConfigs[di].box.l * boxConfigs[di].box.w * boxConfigs[di].box.h;
          if (dv > maxDemandVol) { maxDemandVol = dv; dominantIdx = di; }
        }
        var domBC = boxConfigs[dominantIdx];
        var domKeepUpright = !!(domBC.box && domBC.box.keepUpright);
        var domPackR = calcPacking(c.l, c.w, c.h, domBC.box.l, domBC.box.w, domBC.box.h, gap, 0, allowRotate, domKeepUpright, options);

        if (!domPackR || domPackR.count === 0) {
          // 主导纸箱装不下 → 回退 FFD
          res = calcReverse(c.l, c.w, c.h, boxConfigs, gap, allowRotate, c.maxWeight || 0);
        } else {
          var domMaxPerCrate = domPackR.count;
          var domQtyVal = domBC.qty;
          var domTotalDemand = (domQtyVal === null || domQtyVal === undefined || domQtyVal === '')
            ? null : Math.max(1, parseInt(domQtyVal) || 1);

          if (domTotalDemand === null) domTotalDemand = domMaxPerCrate; // 无限量→只装一个木箱

          var numFullCrates = Math.floor(domTotalDemand / domMaxPerCrate);
          var lastDomCount = domTotalDemand % domMaxPerCrate;

          // 构建次要纸箱配置（排除主导纸箱）
          var secondaryConfigs = [];
          for (var si = 0; si < boxConfigs.length; si++) {
            if (si === dominantIdx) continue;
            secondaryConfigs.push({
              box: boxConfigs[si].box,
              qty: boxConfigs[si].qty
            });
          }

          // 预先计算"满箱"的混装结果（所有 full crate 共用）
          var prePlacedFull = domPackR.positions.slice(0, domMaxPerCrate);
          var fullMixResult = null;
          var secondaryPerFullCrate = {}; // {boxIdx: count}
          if (secondaryConfigs.length > 0) {
            var fullMixConfigs = secondaryConfigs.map(function(sc) { return { box: sc.box, qty: sc.qty }; });
            fullMixResult = calcMixedPacking(c.l, c.w, c.h, fullMixConfigs, gap, allowRotate, 3, 'count', options, prePlacedFull);
            if (fullMixResult && fullMixResult.placed) {
              for (var mi = 0; mi < fullMixResult.placed.length; mi++) {
                var mp = fullMixResult.placed[mi];
                secondaryPerFullCrate[mp.boxIdx] = (secondaryPerFullCrate[mp.boxIdx] || 0) + 1;
              }
            }
          }

          var crates = [];
          var remainingSecondary = {};
          secondaryConfigs.forEach(function(sc, sci) {
            var sq = sc.qty;
            remainingSecondary[sci] = (sq === null || sq === undefined || sq === '') ? 1e9 : Math.max(1, parseInt(sq) || 1);
          });

          // 生成满箱
          for (var fj = 0; fj < numFullCrates; fj++) {
            // 扣除次要纸箱需求
            for (var sci = 0; sci < secondaryConfigs.length; sci++) {
              var used = secondaryPerFullCrate[sci] || 0;
              if (remainingSecondary[sci] < 1e8) remainingSecondary[sci] = Math.max(0, remainingSecondary[sci] - used);
            }
            crates.push(makeHybridCrate(c, domPackR.positions, domBC.box, domMaxPerCrate, fullMixResult, secondaryConfigs));
          }
          ...
          // 溢出处理：检查是否还有未满足的次要纸箱需求
          // 如果末箱装不完，自动追加额外木箱
          if (secondaryConfigs.length > 0) {
            var maxOverflowCrates = 5;
            for (var ofi = 0; ofi < maxOverflowCrates; ofi++) {
              var totalPlaced = {};
              for (var ci = 0; ci < crates.length; ci++) {
                for (var bj = 0; bj < crates[ci].boxes.length; bj++) {
                  var cb = crates[ci].boxes[bj];
                  // 通过名称匹配次要纸箱（而非 boxIdx，因为不同 crate 的 boxIdx 可能不同）
                  for (var sci = 0; sci < secondaryConfigs.length; sci++) {
                    if (secondaryConfigs[sci].box.name === cb.box.name || secondaryConfigs[sci].box.id === cb.box.id) {
                      totalPlaced[sci] = (totalPlaced[sci] || 0) + 1;
                      break;
                    }
                  }
                }
              ...
              var overflowConfigs = [];
              for (var sci2 = 0; sci2 < secondaryConfigs.length; sci2++) {
                var sc = secondaryConfigs[sci2];
                var sq = sc.qty;
                var demand = (sq === null || sq === undefined || sq === '') ? null : Math.max(1, parseInt(sq) || 1);
                if (demand === null) continue;
                var placed = totalPlaced[sci2] || 0;
                var remaining = demand - placed;
                if (remaining > 0) {
                  overflowConfigs.push({ box: sc.box, qty: remaining });
                }
              }
              if (overflowConfigs.length === 0) break;
              var ofResult = calcMixedPacking(c.l, c.w, c.h, overflowConfigs, gap, allowRotate, 3, 'count', options);
              if (!ofResult || !ofResult.placed || ofResult.placed.length === 0) break;
              var ofBoxes = ofResult.placed.map(function(p) {
                var cfg = overflowConfigs[p.boxIdx];
                for (var bci = 0; bci < boxConfigs.length; bci++) {
                  if (boxConfigs[bci].box.name === cfg.box.name) {
                    return { boxIdx: bci, box: boxConfigs[bci].box, pos: p };
                  }
                }
                return { boxIdx: 0, box: cfg.box, pos: p };
              });
              var ofVol = ofBoxes.reduce(function(s, b) { return s + b.pos.l * b.pos.w * b.pos.h; }, 0);
              var ofL = c.l - gap * 2, ofW = c.w - gap * 2, ofH = c.h - gap * 2;
              var ofFV = ofL * ofW * ofH;
              var ofTW = ofBoxes.reduce(function(s, b) { return s + (parseFloat(b.box.weight) || 0); }, 0);
              crates.push({
                crate: { l: c.l, w: c.w, h: c.h },
                boxes: ofBoxes,
                totalCount: ofBoxes.length,
                utilRate: ofFV > 0 ? ofVol / ofFV : 0,
                totalWeight: ofTW,
                maxWeight: c.maxWeight || 0,
                weightRate: c.maxWeight > 0 ? ofTW / c.maxWeight : 0,
                displayCrateL: c.l,
                displayCrateW: c.w,
                displayCrateH: c.h
              });
          ...
          crates.forEach(function(cr) {
            cr.boxes.forEach(function(b) { tv += b.pos.l * b.pos.w * b.pos.h; });
          });
          var iL = c.l - gap * 2, iW = c.w - gap * 2, iH = c.h - gap * 2;
          var fullVol = iL * iW * iH;
          var tc = crates.length;
